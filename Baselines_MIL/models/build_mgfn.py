# ------------------------------------------------------------
"""
This script includes the main class to import and build the MGFN model
...
class MIL_model(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.classifier = import_class('classifier',  config.classifier)(
            embed_dim=config.agent_embed_dim[-1],
            alpha=config.alpha_mgfn,
            drop_rate=0.,
            dim=config.cls_dim,
            drop_rate_instance=config.cls_drop_rate,
            dim_head=config.dim_head_mgfn,
            depths=config.depths_mgfn,
            mgfn_types=config.types_mgfn,
            lokernel=config.lokernel_mgfn,
            ff_repe=config.ff_repe_mgfn,
            attention_drop_rate=config.attention_drop_rate_mgfn
            )
    ...
    def forward(self, x_d, mask_extreme):
        ...
        for i in range(len(mask_extreme)):
            ...
            z_n_feature.append(z_n_feature_i)  # Np, V, T, 64
            z_p_feature.append(z_p_feature_i)  # Np, V, T, 64
...
if __name__ == '__main__':
    ...
    test_mask_extreme = torch.randint(4, (1, 512//4, 832//4)).long().to(device)
    ...
    z_n, z_p, z_n_feature, z_p_feature= model(test_dynamic, test_mask_extreme)

    for i in range(len(z_n)):
        print(z_n[i].shape)
        print(z_p[i].shape)
        print(z_n_feature[i].shape)
        print(z_p_feature[i].shape)
