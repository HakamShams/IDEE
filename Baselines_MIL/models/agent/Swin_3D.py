# ------------------------------------------------------------------
"""
Video Swin Transformer with Cross Attention
Built upon https://github.com/SwinTransformer/Video-Swin-Transformer
...
class WindowAttention3D(nn.Module):
    ...
    def __init__(self, dim, con_dim, window_size, num_heads, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.kv = nn.Linear(con_dim, dim * 2, bias=qkv_bias)
    ...
    def forward(self, x, x_con, mask=None):
        """ Forward function.
        Args:
        ...
        """
        B_, N, C = x.shape
        B_con, N_con, C_con = x_con.shape

        q = self.q(x).reshape(B_, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        kv = self.kv(x_con).reshape(B_con, N_con, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def __init__(self, dim, con_dim, num_heads, window_size=(1, 1,1 ), shift_size=(0, 0, 0),
        ...
        self.norm1_con = norm_layer(con_dim)

        self.attn = WindowAttention3D(
            dim, con_dim, window_size=self.window_size, num_heads=num_heads,
    ...
    def forward_part1(self, x, mask_matrix, x_con):
      ...
      #  x_con = x_con.view(B, D * H * W, C)
        x_con = self.norm1_con(x_con)
      #  x_con = x_con.view(B, D, H, W, C)
        ...
        Bc, Vc, Dc, Hc, Wc, Cc = x_con.shape

        #x_con = x_con.view(Bc, Vc, Dc // window_size[0], window_size[0],
       #                    Hc // window_size[1], window_size[1],
      #                     Wc // window_size[2], window_size[2], Cc)
        #windows_con = x.permute(1, 0, 2, 4, 6, 3, 5, 7, 8).contiguous().view(-1, reduce(mul, window_size), C)

        x_con_windows = x_con.permute(0, 2, 3, 4, 1, 5).contiguous().view(-1, Vc, Cc)

        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, x_con_windows, mask=attn_mask)  # B*nW, Wd*Wh*Ww, C
        # merge windows
    ...
    def forward_part2(self, x):
        B, D, H, W, C = x.shape
        return self.drop_path(self.mlp(self.norm2(x.view(B, D*H*W, C)).view(B, D, H, W, C)))

    def forward(self, x, mask_matrix, x_con):
        """ Forward function.
        ...
        if self.use_checkpoint:
            x = checkpoint.checkpoint(self.forward_part1, x, mask_matrix, x_con)
        else:
            x = self.forward_part1(x, mask_matrix, x_con)

        x = shortcut + self.drop_path(x)

        #print('forward 1 agent', torch.unique(x))

        if self.use_checkpoint:
            x = x + checkpoint.checkpoint(self.forward_part2, x)
        else:
            x = x + self.forward_part2(x)
        #print('forward 2 agent', torch.unique(x))
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, x_con):
        ...
        x_con = rearrange(x_con, 'b v c d h w -> b v d h w c')
...
class Swin_3D(nn.Module):
    """ Swin Transformer backbone with cross attention
    ...
    def __init__(self,
        ...
        self.in_chans = in_chans
        self.embed_dim = embed_dim if embed_dim is not None else [16]
        self.num_layers = len(self.embed_dim)
        self.window_size = window_size if window_size is not None else [(1, 1, 1)]
        self.depths = depths if depths is not None else [1]
        self.num_heads = num_heads if num_heads is not None else [2]
        self.mlp_ratio = mlp_ratio
        ...
        # build layers
        self.layers_var = nn.ModuleList()
        self.proj_var = nn.ModuleList()
        ...
        for _ in range(self.in_vars):
            ...
            for i_layer in range(self.num_layers):
                layer = BasicLayer(
                    ...
                    con_dim=self.embed_dim[i_layer - 1] if i_layer > 0 else self.in_chans,
            ...
            self.proj_var.append(nn.Sequential(nn.Conv3d(self.embed_dim[-1], self.embed_dim[-1], kernel_size=3, stride=1,
                                               ...
                                               nn.Conv3d(self.embed_dim[-1], self.embed_dim[-1], kernel_size=3, stride=1,
                                                         padding=1, padding_mode='replicate', bias=True),
                                               nn.ReLU(),
                      #                         nn.Conv3d(embed_dim[-1], 1, kernel_size=1, stride=1,
                      #                                   padding=0, padding_mode='replicate', bias=True),
                                               )
                                 )
    ...
    def init_weights(self):
        ...
        def _init_weights(m):
            if isinstance(m, nn.Linear) or isinstance(m, nn.Conv2d) or isinstance(m, nn.Conv3d):
                trunc_normal_(m.weight, std=.02)  # 0.05
    ...
    def forward(self, x):
        ...
        for var, layer_var in enumerate(self.layers_var):
            x_v = x[:, var, :, :, :, :]

            index_list = [j for j in range(self.in_vars)]
            index_list.remove(var)
            x_v_con = x[:, index_list, :, :, :, :]

            for layer in layer_var:
                x_v = layer(x_v, x_v_con)
...
if __name__ == '__main__':
    ...
    test_z_q = torch.randn((2, 6, 16, 8, 512//4, 832//4), device=device)
