# ------------------------------------------------------------
"""
This script includes the main class to import and build the ARNet model
https://arxiv.org/abs/2104.07268
...
class MIL_model(nn.Module):
    ...
    def forward(self, x_d, mask_extreme):
        ...
        for i in range(len(mask_extreme)):
            ...
            z_feature_i, z_n_i = self.classifier(z_n_i)  # Nn, V, T, 1
            z_feature_i, z_p_i = self.classifier(z_p_i)  # Np, V, T, 1

            z_n.append(z_n_i)  # Nn, V, T, 1
            z_p.append(z_p_i)  # Np, V, T, 1

            z_feature.append(z_feature_i)  # Np, V, T, 1
...
if __name__ == '__main__':
    ...
    test_mask_extreme = torch.randint(2, (1, 512//4, 832//4)).long().to(device)
