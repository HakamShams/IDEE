# ------------------------------------------------------------------
"""
Script for training and validating on the Synthetic dataset for DeepMIL model
...
from Baselines_MIL.models.losses import RankingLoss, SmoothL2Loss, SparsityLoss
import Baselines_MIL.utils.utils_train as utils
from Baselines_MIL.models.build_deepmil import MIL_model
...
def train(config_file):
    ...
    #criterion_smoothing = SmoothL2Loss(lambda1=config.loss_lambda1).to(device)
    #criterion_sparsity = SparsityLoss(lambda2=config.loss_lambda2).to(device)
    ...
    for epoch in range(config.n_epochs):
        ...
        for i, (data_d, _, _, _, mask_extreme_loss, _, timestep) in tqdm(enumerate(train_dataloader),
            ...
            #loss_ranking, loss_smoothing = torch.zeros(1, device=device), torch.zeros(1, device=device)
            loss_ranking = torch.zeros(1, device=device)

            for k in range(len(z_n)):
                for v in range(config.in_channels_dynamic):
                    loss_ranking += criterion_ranking(z_p[k][:, v, ...], z_n[k][:, v, ...], is_training=True)

                #loss_smoothing += criterion_smoothing(z_p[k])

            #print(loss_ranking, loss_smoothing)
            #loss = (loss_ranking + loss_smoothing) / len(z_n)
            loss = loss_ranking / len(z_n)
            ...
            anomaly = torch.zeros(data_d.shape, requires_grad=False, device=device).squeeze(2)
            # anomaly  N, V, T, H, W
            # z_p  Np, V, T, C
            ...
            anomaly[anomaly > 0.50] = 1
            anomaly[anomaly <= 0.50] = 0
        ...
        with torch.no_grad():
            ...
            for i, (data_d, _, _, _, mask_extreme_loss, mask_anomaly, timestep) in tqdm(enumerate(val_dataloader),
                ...
                for k in range(len(z_n)):
                    for v in range(config.in_channels_dynamic):
                        loss_ranking += criterion_ranking(z_p[k][:, v, ...], z_n[k][:, v, ...])
