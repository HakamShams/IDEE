# ------------------------------------------------------------------
"""
Script for training and validating on the Synthetic dataset for RTFM model
...
from Baselines_MIL.models.losses import RTFMLoss
import Baselines_MIL.utils.utils_train as utils
from Baselines_MIL.models.build_rtfm import MIL_model
import time
import os
import shutil
...
def train(config_file):
    ...
    utils.log_string(logger, "classifier parameters: %d" % (utils.count_parameters(model.classifier) + utils.count_parameters(model.Aggregate)))
    utils.log_string(logger, "all parameters: %d\n" % utils.count_parameters(model))

    # get losses
    utils.log_string(logger, "get criterion ...")

    criterion_rtfm = RTFMLoss(alpha=config.loss_alpha_rtfm,
    ...
    for epoch in range(config.n_epochs):
        ...
        for i, (data_d, _, _, _, mask_extreme_loss, _, timestep) in tqdm(enumerate(train_dataloader),
            ...
            loss_rtfm = torch.zeros(1, device=device)

            for k in range(len(z_n)):
                for v in range(config.in_channels_dynamic):
                    # z_n  Nn, T, 1    z_n_feature  Nn, T, C
                    loss_rtfm += criterion_rtfm(z_p[k][:, v, ...], z_n[k][:, v, ...], z_p_feature[k][:, v, ...],
                                                z_n_feature[k][:, v, ...], is_training=True)
                    #    k=int(torch.sum(mask_extreme_loss[k] == 1) * (1 - config.de_drop_rate)))

            loss = loss_rtfm / len(z_n)
        ...
        with torch.no_grad():
            ...
            for i, (data_d, _, _, _, mask_extreme_loss, mask_anomaly, timestep) in tqdm(enumerate(val_dataloader),
                ...
                for k in range(len(z_n)):
                    for v in range(config.in_channels_dynamic):
                        #              # z_n  Nn, T, 1    z_n_feature  Nn, T, C
                        loss_rtfm += criterion_rtfm(z_p[k][:, v, ...], z_n[k][:, v, ...],
