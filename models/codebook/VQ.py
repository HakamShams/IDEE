...
Proposed in https://arxiv.org/abs/1711.00937
...
if __name__ == '__main__':
    ...
    test_z = torch.randn((2, 32, 6, 512 // 4, 832 // 4), device='cuda')

    model = VQ(dim=32, codebook_size=2, codebook_dim=32).to('cuda')
    print(model)
    ...
    test_z = test_z.view(2, 32, 6 * 512//4 * 832//4).permute(0, 2, 1)

    x, indices, commit_loss = model(test_z)
    x = x.permute(0, 2, 1).view(2, 32, 6, 512//4, 832//4)
