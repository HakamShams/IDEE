...
class LatentQuantize(Module):
    def __init__(
            ...
            dim: int,
            commitment_loss_weight: Optional[float] = 1.,  #0.1
            quantization_loss_weight: Optional[float] = 1.,  # 0.1
            num_codebooks: int = 1,
        ...
        self.register_buffer("commitment_loss_weight", torch.tensor(commitment_loss_weight, dtype=torch.float32),
                             persistent=False)
        self.register_buffer("quantization_loss_weight", torch.tensor(quantization_loss_weight, dtype=torch.float32),
                             persistent=False)
        ...
        values_per_latent = [torch.linspace(-0.5, 0.5, level) if level % 2 == 1 else torch.arange(level) / level - 0.5
                             for level in _levels]  # ensure zero is in the middle and start is always -0.5
        ...
        if optimize_values:
            self.values_per_latent = nn.Parameter(values_per_latent) if self._equal_levels else nn.ParameterList(
            ...
            if exists(in_place_codebook_optimizer):
                self.in_place_codebook_optimizer = in_place_codebook_optimizer(
                    [self.values_per_latent]) if self._equal_levels else in_place_codebook_optimizer(
    ...
    def quantize(self, z: Tensor) -> Tensor:
        ...
        if self._equal_levels:
            ...
            index = torch.stack([torch.argmin(distance(z[..., i, None], self.values_per_latent[i]), dim=-1) for i in
                                 range(self.codebook_dim)], dim=-1)
    ...
    def forward(self,
        ...
        codes = self.quantize(z)  # B, N, 1, number of codebooks
        ...
        # calculate losses
        commitment_loss = self.commitment_loss(original_input,
                                               out) if self.training and self.commitment_loss_weight != 0 else torch.tensor(
            0.)
        quantization_loss = self.quantization_loss(original_input,
                                                   out) if self.training and self.quantization_loss_weight != 0 else torch.tensor(
            0.)
...
if __name__ == '__main__':
    ...
    test_z = torch.randn((2, 16, 8, 512 // 8, 832 // 8), device='cuda')

    model = LatentQuantize(dim=16, levels=[2], num_codebooks=1).to('cuda')
    print(model)

    n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"number of parameters: {n_parameters}")

    x, indices, entropy_aux_loss = model(test_z)
