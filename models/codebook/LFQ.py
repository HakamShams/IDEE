...
Adapted and slightly modified from: https://github.com/lucidrains/vector-quantize-pytorch
...
class LFQ(Module):
    ...
    def forward(
        ...
        if self.training:
            ...
            entropy_aux_loss = self.entropy_loss_weight * per_sample_entropy - codebook_entropy * self.diversity_gamma
        ...
        # complete aux loss
        aux_loss = commit_loss * self.commitment_loss_weight + entropy_aux_loss
...
if __name__ == '__main__':
    ...
    test_z = torch.randn((2, 128, 8, 512 // 8, 832 // 8), device='cuda')

    model = LFQ(dim=128, codebook_size=2 ** 1).to('cuda')
    print(model)
