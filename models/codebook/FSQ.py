...
if __name__ == '__main__':
    ...
    model = FSQ(dim=16, levels=[2]).to('cuda')
    print(model)

    n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"number of parameters: {n_parameters}")

    N, C, T, H, W = test_z.shape
    test_z = test_z.view(N, C, T * H * W).permute(0, 2, 1)
    x, indices = model(test_z)
    x = x.permute(0, 2, 1).view(N, C, T, H, W)
