# ------------------------------------------------------------------
""""
Script for testing on ERA5-Land dataset
...
def test(config_file):
    ...
    test_dataset = ERA5_Land_Dataset(
    ...
    # ensure that the config.en_de_pretrained is not None
    if config.en_de_pretrained is None:
        raise Exception('Testing needs a model checkpoint, config.en_de_pretrained is None')
    ...
    # testing loop
    utils.log_string(logger, 'testing on ERA5 Land dataset ...\n')
    ...
    with torch.no_grad():
        ...
        for i, (data_d, data_drought, data_drought_loss, data_cold_surface, data_cold_surface_loss,
            ...
            pred, pred_y, _, _, _ = model(data_d.to(device))
